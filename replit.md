...
- **Notification Bell Badge**: Header navigation bars display a notification bell icon with a red badge showing unread count, auto-refreshing every 30 seconds (Facebook-style).
- **Image Editor Flow**: After capturing photo/video in camera, user is directed to ImageEditorScreen with professional editing tools before posting to story or feed.
...
- **Key Features**:
  ...
  - **AI Content Generation**: Gemini AI integration (`/api/ai/generate-text`) with two modes: `improve` enhances existing text to be more engaging, `generate` creates new content from user prompts. Uses Gemini 2.0 Flash model for fast, high-quality Vietnamese content generation.
...
- **Google Gemini AI**: AI-powered content generation using Gemini 2.0 Flash model via `@google/genai` package. Improves existing text and generates new content from prompts in Vietnamese.
