...
const getLikedPosts = async (req, res) => {
  ...
  const cacheKey = cacheService.getCacheKey('likedposts', userId, currentUserId);
    ...
    const result = await pool.queryAll(
      `WITH liked_post_ids AS (
       ...
       visible_posts AS (
         SELECT p.id, p.user_id, p.content, p.media_url, p.media_type, p.media_width, p.media_height, p.privacy, p.created_at, p.updated_at
         FROM posts p
         WHERE p.id IN (SELECT post_id FROM liked_post_ids) AND (
           p.user_id = $2
           OR p.privacy = 'public'
           OR (p.privacy = 'friends' AND EXISTS (
             SELECT 1 FROM friendships 
             WHERE status = 'accepted' 
             AND ((requester_id = p.user_id AND addressee_id = $2) OR (requester_id = $2 AND addressee_id = p.user_id))
           ))
         )
         ORDER BY p.created_at DESC
       ),
