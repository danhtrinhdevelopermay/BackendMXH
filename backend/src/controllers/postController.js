...
const getLikedPosts = async (req, res) => {
  ...
  const cacheKey = cacheService.getCacheKey('likedposts', user_id);
    ...
    const result = await pool.queryAll(
      `WITH liked_post_ids AS (
       ...
       visible_posts AS (
         SELECT p.id, p.user_id, p.content, p.media_url, p.media_type, p.media_width, p.media_height, p.privacy, p.created_at, p.updated_at
         FROM posts p
         WHERE p.id IN (SELECT post_id FROM liked_post_ids)
         AND (
           p.user_id = $1
           OR p.privacy = 'public'
           OR (p.privacy = 'friends' AND EXISTS (
             SELECT 1 FROM friendships 
             WHERE status = 'accepted' 
             AND ((requester_id = p.user_id AND addressee_id = $1) OR (requester_id = $1 AND addressee_id = p.user_id))
           ))
         )
       ),
       reaction_counts AS (
...
const getSharedPosts = async (req, res) => {
  ...
  const cacheKey = cacheService.getCacheKey('sharedposts', user_id, limit, offset);
  ...
  try {
    ...
    const result = await pool.queryAll(
      `WITH shared_post_ids AS (
         SELECT DISTINCT 
           CAST(SUBSTRING(m.content FROM '/post/(\d+)') AS INTEGER) as post_id
         FROM messages m
         WHERE m.receiver_id = $1 AND m.content LIKE '%/post/%'
       ),
       visible_posts AS (
         SELECT p.id, p.user_id, p.content, p.media_url, p.media_type, p.media_width, p.media_height, p.privacy, p.created_at, p.updated_at
         FROM posts p
         WHERE p.id IN (SELECT post_id FROM shared_post_ids)
         AND (
           p.user_id = $1
           OR p.privacy = 'public'
           OR (p.privacy = 'friends' AND EXISTS (
...
module.exports = { createPost, getNewsFeed, getUserPosts, deletePost, searchPosts, getLikedPosts, getSharedPosts };
